def generate_home_view(client, event, logger, airtable):
    user = airtable.get_user(event["user"])
    if not user:
        user = {
            "fields": {
                "User ID": event["user"],
                "Name": client.users_info(user=event["user"])["user"]["real_name"],
            }
        }
        airtable.update_or_add_user(
            event["user"], user["fields"]["Name"], whitelisted=False, admin=False
        )
    if user["fields"].get("Admin") or user['fields'].get('Allowed to Phone'):
        view = {
            "type": "home",
            "blocks": [
                {
        ...
        client.chat_postMessage(
            channel=event["user"],
            text="You are not allowed to use this app. Please contact an admin.",
        )
